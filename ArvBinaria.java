...
public class ArvBinaria<Chave extends Comparable<Chave>, Valor>
{
    ...
    //Letra d, porquê o 8 é maior que o 7, então ao invés dele se tornar um filho do 3 e continuar o caminho, ele criaria outra subárvore a partir do 7.
    ...
    public void constroiArvore(int[] vetor){
        if(vetor == null)
            ...
            for(int i = 0; i< vetor.length;i++) {
                raiz = constroiArvore(raiz, vetor, i);
            }
        }
    }
    //O(n), pq o for roda toda o vetor.

    private ArvBinaria.No constroiArvore(No raiz, int[] vetor, int n){
        if(raiz == null) {
            return new No((Chave) (Integer) vetor[n], (Valor) ((Integer) vetor[n]));
        }
        int data = vetor[n];
        int raizData = (Integer)raiz.valor;
        if(data > raizData){
            raiz.dir = constroiArvore(raiz.dir, vetor, n);
        }
        if(data < raizData){
            raiz.esq = constroiArvore(raiz.esq, vetor, n);
        }
        return raiz;
    }
    //O(log n), já que ele percorre a árvore de forma recursiva
